...
pub fn depth_first_search(graph: &Graph, root: Vertex, objective: Vertex) -> Option<Vec<u32>> {
    ...
    while let Some(current_vertex) = queue.pop_front() {
        ...
        // for each over the neoghbors of current vertex
        for neighbor in current_vertex.neighbors(graphs).into_iter().rev() {
            // insert in the hashset of visited if this value not exist yet
            if visited.insert(neighbor) {
                // add the neoghbor to the queue
